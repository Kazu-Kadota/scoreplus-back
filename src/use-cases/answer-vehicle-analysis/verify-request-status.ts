import { ValidateSendAnswerVehicleBody } from '../../controllers/requestplus/send-answer/vehicle/validate-body'
import { CompanyRequestVehicleConfigEnum } from '~/models/dynamo/enums/company'
import { RequestStatusEnum, VehicleAnalysisStateEnum } from '~/models/dynamo/enums/request'
...
const verifyRequestStatus = ({
  ...
  const is_processing_or_waiting = request_vehicle_status.general === RequestStatusEnum.WAITING
    || request_vehicle_status.general === RequestStatusEnum.PROCESSING
  ...
  for (const answer of answers_body) {
    const is_eagle_request = answer.type === CompanyRequestVehicleConfigEnum.ANTT
      || answer.type === CompanyRequestVehicleConfigEnum.BASIC_DATA
    ...
    if (answer.type === CompanyRequestVehicleConfigEnum.PLATE_HISTORY) {
      ...
      plate_history_answer.regions[index_find_region][region] = RequestStatusEnum.VALIDATING
  ...
  */
  ...
  if (is_worker_finished) {
    const vehicle_analysis_options_answered = mount_vehicle_analysis_options as Partial<VehicleAnalysisOptionsRequest<true>>
    return {
      is_finished: true,
      status: analysis_answered,
      vehicle_analysis_options: vehicle_analysis_options_answered,
      // is_all_approved,
    }
  }

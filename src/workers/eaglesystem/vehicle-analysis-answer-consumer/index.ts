...
import eagleSystemVehicleAnalysisAnswerConsumerWorker from './main'

export const handler = (event: SQSEvent) => {
  logger.setService('requestplus')

  const releaseExtract = new LambdaHandlerNameSpace
    .LambdaSQSHandlerFunction<DatavalidSQSReceivedMessageAttributes>(eagleSystemVehicleAnalysisAnswerConsumerWorker)

...
import { M2VehicleAnalysisTypeEnum } from '~/models/m2system/enums/analysis'
...
const m2SystemVehicleAnalysisAnswerConsumerWorker: SQSController = async (message) => {
  logger.debug({
    message: 'Start on answer request vehicle analysis from m2system response',
  })
  ...
  const company_request_vehicle = m2SystemScoreplusVehicleConfigMap[body.detail.vehicle_analysis_type as M2VehicleAnalysisTypeEnum] as CompanyRequestVehicleConfigEnum
  ...
  logger.info({
    message: 'Finish on answer request vehicle analysis from m2system response',
    vehicle_id,
  })
...
export default m2SystemVehicleAnalysisAnswerConsumerWorker

import { PFBasicBiometryAcceptablePercentageEnum } from '~/models/datavalid/pf-basic/biometry-acceptable-percentage'
...
const verifyResult = (body: PFBasicResult): VerifyResultReturn => {
  logger.debug({
    message: 'Verifying results of pf-basic',
  })
  ...
  if (body.filiacao.nome_mae_similaridade && body.filiacao.nome_mae_similaridade < PFBasicBiometryAcceptablePercentageEnum.PERCENTAGE) {
    reproved_data.set('filiacao.nome_mae_similaridade', 'reproved')
  }

  if (body.filiacao.nome_pai_similaridade && body.filiacao.nome_pai_similaridade < PFBasicBiometryAcceptablePercentageEnum.PERCENTAGE) {

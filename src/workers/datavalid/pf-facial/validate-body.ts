...
import { CNHAnswer, DocumentoAnswer, EnderecoAnswer, FiliacaoAnswer } from '~/models/datavalid/pf-basic/request-body'
import { PFFacialSendRequestAnswer, PFFacialBiometrySendRequestBody, PFFacialKey } from '~/models/datavalid/pf-facial/request-body'
...
// const FaceAnswerSchema = Joi.object({
//   formato: Joi
//     .string()
//     .valid('JPG', 'PDF', 'PNG', 'jpg', 'pdf', 'png')
//     .optional(),
//   base64: Joi
//     .string()
//     .optional(),
...
const FaceAnswerSchema = Joi.object<DatavalidRequestImage, true>({
  s3_image_path: Joi
    .string()
    .required(),
})

const PFFacialAnswerSchema = Joi.object<PFFacialSendRequestAnswer, true>({
  nome: Joi
    .string()
    .max(255)
    .optional(),
  ...
  biometria_face: FaceAnswerSchema.required(),
})

const schema = Joi.object<PFFacialBiometrySendRequestBody, true>({
  key: PFKeySchema.required(),
  answer: PFFacialAnswerSchema.required(),
}).required()

const validateBody = (
  data: Partial<PFFacialBiometrySendRequestBody>,
  ...
  if (error) {
    logger.error({
      message: 'Error on validate send request facial biometry',
    })

    throw new BadRequestError('Erro na validação do body para envio da análise de biometria facial', error.stack as string)
  }

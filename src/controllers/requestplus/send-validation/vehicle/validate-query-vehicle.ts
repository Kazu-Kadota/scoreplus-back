...
const validateQueryVehicle = (
  ...
  if (error) {
    logger.error({
      message: 'Error on validate "send validate vehicle" query',
    })

    throw new BadRequestError('Erro na validação do query para envio da validation da análise de veículo', error.stack as string)
  }

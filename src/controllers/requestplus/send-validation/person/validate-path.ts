...
const validatePath = (
  ...
  if (error) {
    logger.error({
      message: 'Error on validate "send validate person" path',
    })

    throw new BadRequestError('Erro na validação do path para envio da validação da análise de pessoas', error.stack as string)
  }

...
export const handler = async (
  ...
  try {
    ...
    if (user_info.user_type === 'operator') {
      throw new ErrorHandler('Usuário não autorizado para requisitar análises', 401)
    }

    logger.debug({
      message: 'Start request analysis path',
      user_info,
    })
    const result = await requestAnalysis(event, user_info)

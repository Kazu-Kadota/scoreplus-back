...
import { BiometryFacialParseBodyReturn } from './parse-body'
...
const schema = Joi.object<BiometryFacialParseBodyReturn, true>({
  ...
  facial_image_name: Joi.string().max(255).required(),
  person: person_schema.required(),
  person_analysis_type: person_analysis_type_schema.optional(),
}).required()

const validateBody = (
  data: Partial<BiometryFacialParseBodyReturn>,
): BiometryFacialParseBodyReturn => {
  ...
  if (error) {
    logger.error({
      message: 'Error on validate request person biometry facial body',
    })

    throw new BadRequestError('Erro na validação do body para solicitação de análise de biometria facial de pessoa', error.stack as string)
  }

...
const getRequestPersonAdapter = async ({
  ...
  if (request_person.status['biometry-facial']) {
    logger.warn({
      message: 'Already requested to analyze biometry-facial',
      request_id: key.request_id,
      person_id: key.person_id,
    })

    throw new ConflictError('Já solicitado para realizar análise de biometria facial')
  }
